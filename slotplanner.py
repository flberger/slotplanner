...
AUTOCOMPLETE_CSS = """.autocomplete-suggestions {
...
""".format(first_name.strip(), last_name.strip(), twitter_handle.strip(), title.strip(), VERSION, self.config["event"])
    ...
    def schedule(self, **kwargs):
        ...
        page = simple.html.Page("Schedule Contributions", css = self.config["page_css"] + MENU_CSS + AUTOCOMPLETE_CSS)
        ...
        for i in range(len(self.slotplanner_db["slot_dimension_names"][0])):
            ...
            form.add_input("", "text", "contribution", html_id="contributions_{}".format(i))
        ...
        # Monkey-patch scripts into the page
        #
        autocomplete_contributions = """var autocomplete_contributions = new autoComplete({
            selector: '#HTML_ID',
            minChars: 1,
        ...
        # .format() doesn't work
        #
        autocomplete_contributions = autocomplete_contributions.replace("CHOICES", str(contributions))

        autocomplete_scripts = ""
        
        for i in range(len(self.slotplanner_db["slot_dimension_names"][0])):

            autocomplete_scripts += autocomplete_contributions.replace('HTML_ID', "contributions_{}".format(i))

        page_str = page_str.replace('</body>', '<script>{}</script><script>{}</script></body>'.format(AUTOCOMPLETE, autocomplete_scripts))
